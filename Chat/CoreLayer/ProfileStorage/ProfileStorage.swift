...
class ProfileStorage: IProfileStorage {
    ...
    func fetchAvatar() -> UIImage? {
        guard let data = cacheStorage.fetch(by: ProfileKeys.avatar.rawValue) as? Data, let image = UIImage(data: data) else { return nil }
        
        return image
    }
    
    private func fetchString(by key: ProfileKeys) -> String? {
        guard let data = cacheStorage.fetch(by: key.rawValue) as? Data, let value = String(data: data, encoding: .utf8) else {
            return nil
        }
