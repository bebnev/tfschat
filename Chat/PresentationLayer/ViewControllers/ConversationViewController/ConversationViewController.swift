...
class ConversationViewController: AbstractViewController {
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(ConversationMessageTableViewCell.self, forCellReuseIdentifier: String(describing: ConversationMessageTableViewCell.self))
    ...
    func handleSendTap() {
        ...
        guard let content = inputField.text?.trimmingCharacters(in: .whitespacesAndNewlines),
            let senderName = presentationAssembly?.profileManager.profile?.name,
            let userId = presentationAssembly?.profileManager.profile?.id,
            let channelIdentifier = channel?.identifier else {
            return
        }
        ...
        let data: [String: Any] = ["content": content, "senderId": userId, "senderName": senderName, "created": Timestamp(date: Date())]
        dataService?.addMessage(for: channelIdentifier, data: data) {[weak self] error in
            if error == nil {
